...
sql.execute("""CREATE TABLE IF NOT EXISTS buys (name TEXT, buyer BIGINT, price INT, tovarid INT, date DATE)""")

db.commit()

@client.message_handler(commands=['report'])
def report(message):
	try:
		cid = message.chat.id
		...
		if getaccess < accessquery:
			...
			msg = client.send_message(cid, f"Введите начальную дату отчёта в формате ГГГГ-ММ-ДД", parse_mode='Markdown')
			client.register_next_step_handler(msg, report_start_date)
	except:
		client.send_message(cid, f'🚫 | Ошибка при выполнении команды')

def report_start_date(message):
	try:
		cid = message.chat.id
		uid = message.from_user.id
		if message.text == message.text:
			...
			msg = client.send_message(cid, 'Введите конечную дату отчёта в формате ГГГГ-ММ-ДД', parse_mode='Markdown')
			client.register_next_step_handler(msg, report_end_date)
	except:
		client.send_message(cid, f'🚫 | Ошибка при выполнении команды')

def report_end_date(message):
	try:
		cid = message.chat.id
		uid = message.from_user.id
		if message.text == message.text:
			...
			elif start_date.lower() == 'год назад':
				start_date = str(datetime.date.today() - datetime.timedelta(days = 365))
			elif start_date.lower() == 'начало года':
				start_date = str(datetime.date(int(str(datetime.date.today()).split('-')[0]),1,1))
			elif start_date.lower() == 'конец года':
				start_date = str(datetime.date(int(str(datetime.date.today()).split('-')[0]),12,31))
			if end_date.lower() == 'сегодня':
				end_date = str(datetime.date.today())
			elif end_date.lower() == 'год назад':
				end_date = str(datetime.date.today() - datetime.timedelta(days = 365))
			elif end_date.lower() == 'начало года':
				end_date = str(datetime.date(int(str(datetime.date.today()).split('-')[0]),1,1))
			elif end_date.lower() == 'конец года':
				end_date = str(datetime.date(int(str(datetime.date.today()).split('-')[0]),12,31))
			if start_date <= end_date:
				client.send_message(cid, f'📅 | Отчет по продажам услуг с *{start_date}* по *{end_date}*:', parse_mode='Markdown')
				sql.execute(f"SELECT MAX(id) FROM shop")
				items_count = int(list(sql.fetchone())[0])
				db.commit()
				for item_id in range(items_count):
					item_position = item_id + 1
					sql.execute(f"SELECT name, SUM(price) FROM buys WHERE date >= '{start_date}' AND date <= '{end_date}' AND tovarid = {item_position}")
					report_list = list(sql.fetchone())
					db.commit()
					if report_list[1] != None:
						report_name = str(report_list[0])
						report_price = int(report_list[1])
						client.send_message(cid, f'Товар "{report_name}" был продан на сумму {report_price} руб.', parse_mode='Markdown')
			else:
				client.send_message(cid, f'🚫 | Ошибка! Неверно задан временной диапазон')
	except:
		client.send_message(cid, f'🚫 | Ошибка при выполнении команды')
...
def myprofile(message):
	try:
		...
		for info in sql.execute(f"SELECT * FROM users WHERE id = {uid}"):
			client.send_message(cid, f"*📇 | Твой профиль:*\n\n*👤 | Ваш ID:* {info[0]}\n*💸 | Уровень доступа:* {accessname}\n*🛒 | Куплено услуг:* {info[4]}\n\n*🗂 Чтобы посмотреть список купленных услуг напишите /mybuy*", parse_mode='Markdown')
	except:
		client.send_message(cid, f'🚫 | Ошибка при выполнении команды')
...
def editbuy(message):
	try:
		...
		if getaccess < 1:
			...
			item_tovar = types.InlineKeyboardButton(text='Услуга',callback_data='editbuytovar')
...
def removebuy_callback(call):
	try:
		if call.data == 'removebuytovaryes':
			...
			client.delete_message(call.message.chat.id, call.message.message_id-0)
			client.send_message(call.message.chat.id, f"✅ | Вы успешно удалили услугу")
			db.commit()
...
def addbuy(message):
	try:
		...
		if getaccess < 1:
			...
			msg = client.send_message(cid, '*Введите ID услуги:*',parse_mode='Markdown')
...
def addbuy_id(message):
	try:
		...
		if message.text == message.text:
			...
			msg = client.send_message(cid, '*Введите цену услуги:*',parse_mode='Markdown')
...
def addbuy_price(message):
	try:
		...
		if message.text == message.text:
			...
			msg = client.send_message(cid, '*Введите название услуги:*',parse_mode='Markdown')
...
def addbuy_name(message):
	try:
		...
		if message.text == message.text:
			...
			msg = client.send_message(cid, '*Введите ссылку на услугу:*',parse_mode='Markdown')
	...
	try:
		...
		if message.text == message.text:
			...
			if sql.fetchone() is None:
				...
				client.send_message(cid, f'✅ | Вы успешно добавили услугу\nID услуги: {sql.fetchone()[0]}\nИмя: {addbuyname}\nЦена: {addbuyprice}\nСсылка на услугу: {addbuytovar}')
			else:
				client.send_message(cid, f"⛔️ | Данная услуга уже добавлена!")
	except:
		client.send_message(cid, f'🚫 | Ошибка при выполнении команды')
...
def buy(message):
	try:
		...
		for info in sql.execute(f"SELECT * FROM users WHERE id = {uid}"):
			...
			rmk.add(item_yes, item_no)
			msg = client.send_message(cid, f'{text}*Вы хотите перейти к покупке услуги?*',parse_mode='Markdown',reply_markup=rmk)
	except:
		client.send_message(cid, f'🚫 | Ошибка при выполнении команды')
		...
		if message.text == message.text:
			...
			for info in sql.execute(f"SELECT * FROM users WHERE id = {uid}"):
				for infoshop in sql.execute(f"SELECT * FROM shop WHERE id = {tovarid}"):
					...
					rmk.add(item_yes, item_no)
					msg = client.send_message(cid, f"💸 | Вы подверждаете покупку услуги?",reply_markup=rmk)
	except:
		client.send_message(cid, f'🚫 | Ошибка при выполнении команды')
...
def firstbuy_callback(call):
	try:
		if call.data == 'firstbuytovaryes':
			msg = client.send_message(call.message.chat.id, f"*Введите ID услуги который хотите купить:*",parse_mode='Markdown')
			client.register_next_step_handler(msg, buy_next)
		elif call.data == 'firstbuytovarno':
			client.delete_message(call.message.chat.id, call.message.message_id-0)
			client.send_message(call.message.chat.id, f"🚫 | Вы отменили покупку услуги")
		client.answer_callback_query(callback_query_id=call.id)
	except:
		client.send_message(call.message.chat.id, f'🚫 | Ошибка при выполнении команды')
			...
			for info in sql.execute(f"SELECT * FROM users WHERE id = {call.from_user.id}"):
				for infoshop in sql.execute(f"SELECT * FROM shop WHERE id = {tovarid}"):
					if True:
						...
						tovar_name = str(infoshop[1])
						tovar_price = int(infoshop[2])
						current_date = str(datetime.date.today())
						sql.execute(f"SELECT * FROM users WHERE id = {call.from_user.id}")
						client.delete_message(call.message.chat.id, call.message.message_id-0)
						client.send_message(call.message.chat.id, f"✅ | Вы успешно отправили заявку на покупку услуги.\nМы свяжемся с вами в ближайшее время \n\n{infoshop[1]}\nЦена: {infoshop[2]}₽\nУслуга: {infoshop[3]}\n\nСпасибо за покупку!")
						client.send_message(718552446, f"✉️ | Пользователь {info[1]} отправил заявку на покупку услуги\nID пользователя: {call.from_user.id}\nНаименование услуги: {infoshop[1]} \nУслуга: {infoshop[3]}\nСумма: {infoshop[2]}₽\n\nДанный пользователь приобрел услуг: {info[4]}")
						...
						db.commit()
						if sql.fetchone() is None:
							sql.execute(f"INSERT INTO buys VALUES ('{tovar_name}', {call.from_user.id}, {tovar_price}, {tovarid}, '{current_date}')")
							db.commit()						
					else:
						client.delete_message(call.message.chat.id, call.message.message_id-0)
						client.send_message(call.message.chat.id, f"*⛔️ | Данная услуга уже куплена!*\n\nЧтобы посмотреть список купленных услуг напишите /mybuy",parse_mode='Markdown')
		elif call.data == 'buytovarno':
			client.delete_message(call.message.chat.id, call.message.message_id-0)
			client.send_message(call.message.chat.id, f"❌ | Вы отменили покупку услуги!")
		client.answer_callback_query(callback_query_id=call.id)
	except:
		client.send_message(call.message.chat.id, f'🚫 | Ошибка при выполнении команды') #{traceback.format_exc()}
...
def helpcmd(message):
	...
	if getaccess >= 1:
		...
		client.send_message(cid, '*Помощь по командам:*\n\n/profile - Посмотреть свой профиль\n/help - Посмотреть список команд\n/buy - Купить услугу\n/mybuy - Посмотреть список купленных услуг\n/teh - Связаться с тех.поддержкой\n\nАдмин-команды:\n\n/getprofile - Посмотреть чужой профиль\n/access - Выдать уровень доступа\n/getid - Узнать айди пользователя\n/getcid - Узнать Conference ID\n/addbuy - Добавить услугу на продажу\n/editbuy - Изменить данные о товаре\n/rembuy - Удалить услугу\n/ot - Ответить пользователю (отправить сообщение)\n/report - Запросить отчет за определённый промежуток времени',parse_mode='Markdown')
	else:
...
def getrazrabotchik(message):
	if message.from_user.id == 718552446:
		sql.execute(f"UPDATE users SET access = 777 WHERE id = 718552446")
	...
	try:
		if call.data == 'tehsend':
			for info in sql.execute(f"SELECT * FROM users WHERE id = {call.from_user.id}"):
				...
				client.send_message(718552446, f"✉️ | Пользователь {tehnamebyuser} отправил сообщение в тех.поддержку\n\nID пользователя: {tehidbyuser}\nТекст: {tehtextbyuser}\n\nЧтобы ответить пользователю напишите /ot")
